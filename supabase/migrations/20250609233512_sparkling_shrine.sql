/*
  # User Data Tracking and Historical Performance - Fixed
  ...
  2. Security
    ...
    - Add policies for authenticated users to access their own data (only if they don't exist)
*/
...
BEGIN
  ...
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'audit_history') THEN
  ...
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_activity') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'audit_history' AND policyname = 'Users can read own audit history') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'audit_history' AND policyname = 'Users can insert own audit history') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_history_user_id') THEN
    CREATE INDEX idx_audit_history_user_id ON audit_history(user_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_history_created_at') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_reports_user_id') THEN
