...
const getContentGenerationPrompt = (request)=>{
  ...
  const entityInstruction = entitiesToInclude && entitiesToInclude.length > 0 ? `**Important: You must naturally integrate these entities into the content:** ${entitiesToInclude.join(', ')}.` : '';
  let specificFormat = '';
  switch(contentType){
    case 'faq':
      specificFormat = `
...
export const contentGeneratorService = async (req, supabase)=>{
  ...
  try {
    ...
    if (!projectId) {
      throw new Error("projectId is required for logging.");
    }
    ...
    // Gemini API call with the same model as working audit
    ...
    if (!geminiResponse.ok) {
      ...
      throw new Error(`The AI model failed to process the request. Status: ${geminiResponse.status}. Body: ${errorText}`);
    }
