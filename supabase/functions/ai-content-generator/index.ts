...
export const contentGeneratorService = async (req: Request, supabase: SupabaseClient): Promise<Response> => {
    ...
    try {
        ...
        runId = await logToolRun({ supabase, projectId, toolName: 'ai-content-generator', inputPayload });
        ...
        if (runId) {
            await updateToolRun({ supabase, runId, status: 'completed', outputPayload: generatedJson, errorMessage: null });
        }
